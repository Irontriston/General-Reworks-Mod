...
local TargetLaser = import('/lua/kirvesweapons.lua').TargetingLaserInvisible
local Buff = import('/lua/sim/Buff.lua')

UAL0001 = Class(ACUUnit) {
    Weapons = {
		ViewTarget = Class(TargetLaser) {},
    ...
    CreateEnhancement = function(self, enh)
        ...
        elseif enh == 'CrysalisBeam' then
			...
			local head = self:GetWeaponByLabel('ViewTarget')
        ...
        elseif enh == 'HeatSink' then
            local wep = self:GetWeaponByLabel('RightDisruptor')
            wep:ChangeRateOfFire(bp.NewRateOfFire or 3)
			wep:AddDamageMod(bp.DamageAmplifier)
        elseif enh == 'HeatSinkRemove' then
            local wep = self:GetWeaponByLabel('RightDisruptor')
            local bpDisrupt = self:GetBlueprint().Weapon[1].RateOfFire
            wep:ChangeRateOfFire(bpDisrupt or 1)
			wep:AddDamageMod(-bp.DamageModifier)
