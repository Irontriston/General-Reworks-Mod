...
local HeadTargeting = import('/lua/kirvesweapons.lua').TargetingLaserInvisible
...
UEL0001 = Class(ACUUnit) {
    Weapons = {
		...
		HeadTargetter = Class(HeadTargeting) {},
    ...
    __init = function(self)
        ACUUnit.__init(self, 'HeadTargetter')
    end,
    ...
    RebuildPod = function(self, PodNumber)
        if PodNumber == 1 then
            ...
            if self.HasLeftPod == true then
                self.RebuildingPod = CreateEconomyEvent(self, 1600, 160, 5, self.SetWorkProgress)
        ...
        elseif PodNumber == 2 then
            ...
            if self.HasRightPod == true then
                self.RebuildingPod2 = CreateEconomyEvent(self, 1600, 160, 5, self.SetWorkProgress)
                self:RequestRefreshUI()
                WaitFor(self.RebuildingPod2)
                self:SetWorkProgress(0.0)
    ...
    CreateEnhancement = function(self, enh)
        ...
        elseif enh =='HeavyAntiMatterCannon' then
			...
			wep:ChangeRateOfFire(bp.NewRateOfFire or 1.25)
            --local wep2 = self:GetWeaponByLabel('LeftZephyr')
            --wep2:AddDamageMod(bp.ZephyrDamageMod)
            --wep2:ChangeMaxRadius(bp.NewMaxRadius or 44)
			--wep2:ChangeRateOfFire(bp.NewRateOfFire or 1.25)
			local head = self:GetWeaponByLabel('HeadTargetter')
			head:ChangeMaxRadius(bp.NewMaxRadius or 44)
            local oc = self:GetWeaponByLabel('OverCharge')
            oc:ChangeMaxRadius(bp.NewMaxRadius or 44)
			oc:ChangeRateOfFire(bp.NewRateOfFire or 1.25)
            local aoc = self:GetWeaponByLabel('AutoOverCharge')
            aoc:ChangeMaxRadius(bp.NewMaxRadius or 44)
			aoc:ChangeRateOfFire(bp.NewRateOfFire or 1.25)
        elseif enh =='HeavyAntiMatterCannonRemove' then
            local bp2 = self:GetBlueprint().Enhancements['HeavyAntiMatterCannon']
            if not bp2 then return end
            local bpDisrupt = self:GetBlueprint().Weapon[1].MaxRadius
			local bpROF = self:GetBlueprint().Weapon[1].RateOfFire
            local wep = self:GetWeaponByLabel('RightZephyr')
            wep:AddDamageMod(-bp2.ZephyrDamageMod)
            wep:ChangeMaxRadius(bpDisrupt or 22)
			wep:ChangeRateOfFire(bpROF or 1)
            --local wep2 = self:GetWeaponByLabel('LeftZephyr')
            --wep2:AddDamageMod(-bp2.ZephyrDamageMod)
            --wep2:ChangeMaxRadius(bpDisrupt or 22)
			--wep2:ChangeRateOfFire(bpROF or 1)
			local head = self:GetWeaponByLabel('HeadTargetter')
			head:ChangeMaxRadius(bpDisrupt or 22)
            local oc = self:GetWeaponByLabel('OverCharge')
            oc:ChangeMaxRadius(bpDisrupt or 22)
			oc:ChangeRateOfFire(bpROF or 1)
            local aoc = self:GetWeaponByLabel('AutoOverCharge')
            aoc:ChangeMaxRadius(bpDisrupt or 22)
			aoc:ChangeRateOfFire(bpROF or 1)
