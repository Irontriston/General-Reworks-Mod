...
UEB1101 = Class(TEnergyCreationUnit) {
    ...
    OpeningState = State {
        Main = function(self)
            ...
            self.Sliders.Slider1:SetGoal(0, 0, -0.1)
            self.Sliders.Slider1:SetSpeed(0.15)
            self.Sliders.Slider2:SetGoal(-0.1, 0, 0)
            self.Sliders.Slider2:SetSpeed(0.15)
            self.Sliders.Slider3:SetGoal(0, 0, 0.1)
            self.Sliders.Slider3:SetSpeed(0.15)
            self.Sliders.Slider4:SetGoal(0.1, 0, 0)
            self.Sliders.Slider4:SetSpeed(0.15)
            for k, v in self.Sliders do
                WaitFor(v)
            end
            for k, v in self.Spinners do
    ...
    ClosedIdleState = State {
        Main = function(self)
            while self.DamageSeconds > 0 do
                WaitSeconds(0.2)
                self.DamageSeconds = self.DamageSeconds - 0.2
            end
            ChangeState(self, self.OpeningState)
        end,
