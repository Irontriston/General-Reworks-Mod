...
URL0001 = Class(ACUUnit, CCommandUnit) {
    Weapons = {
		...
		ViewTarget = Class(HeadTargeting) {},
    ...
    __init = function(self)
        ACUUnit.__init(self, 'ViewTarget')
    end,
    ...
    CreateEnhancement = function(self, enh)
        ...
        elseif enh =='CoolingUpgrade' then
            ...
            wep:ChangeMaxRadius(bp.NewMaxRadius or 42)
            self.normalRange = bp.NewMaxRadius or 42
            wep:ChangeRateOfFire(bp.NewRateOfFire or 2)
            local microwave = self:GetWeaponByLabel('MLG')
            microwave:ChangeMaxRadius(bp.NewMaxRadius or 42)
            local oc = self:GetWeaponByLabel('OverCharge')
            oc:ChangeMaxRadius(bp.NewMaxRadius or 42)
            local aoc = self:GetWeaponByLabel('AutoOverCharge')
            aoc:ChangeMaxRadius(bp.NewMaxRadius or 42)
            if not (self:GetCurrentLayer() == 'Seabed' and self:HasEnhancement('NaniteTorpedoTube')) then
                self:GetWeaponByLabel('ViewTarget'):ChangeMaxRadius(self.normalRange)
            end
        ...
        elseif enh == 'NaniteTorpedoTube' then
            ...
            if self:GetCurrentLayer() == 'Seabed' then
                self:GetWeaponByLabel('ViewTarget'):ChangeMaxRadius(self.torpRange)
            end
    ...
    OnKilled = function(self, instigator, type, overkillRatio)
        ...
        ACUUnit.OnKilled({Unit = self, Proj = 'CybranDeath'}, instigator, type, overkillRatio)
    end,

    OnLayerChange = function(self, new, old)
        ACUUnit.OnLayerChange(self, new, old)
        if not self:GetWeaponByLabel('ViewTarget') then return end
        if new == "Seabed" and self:HasEnhancement('NaniteTorpedoTube') then
            self:GetWeaponByLabel('ViewTarget'):ChangeMaxRadius(self.torpRange or 60)
        else
            self:GetWeaponByLabel('ViewTarget'):ChangeMaxRadius(self.normalRange or 22)
        end
    end,
}
