...
XRL0403 = Class(CWalkingLandUnit) {
    ...
    Weapons = {
        ...
        HackPegLauncher= Class(CDFBrackmanCrabHackPegLauncherWeapon)
		{
			CreateProjectileForWeapon = function(self, bone)
				local proj = CDFBrackmanCrabHackPegLauncherWeapon.CreateProjectileForWeapon(self, bone)
				return proj:SetNewTargetGround(self:GetCurrentTargetPos() or self:GetCurrentTarget().Position(0) )--SetTargetPosition seems to not work.
			end
		},
    },

    DisableAllButHackPegLauncher= function(self)
        self:SetWeaponEnabledByLabel('ParticleGunRight', false) -- Disables all other weapons.
    ...
    EnableHackPegLauncher= function(self)
        self:SetWeaponEnabledByLabel('HackPegLauncher', true)   -- Enables hack-peg launcher.
    end,
    ...
    DeathThread = function(self)
        ...
        for i, numWeapons in bp.Weapon do
            if(bp.Weapon[i].Label == 'MegalithDeath') then
                ...
                DamageArea({ Unit = self, Proj = bp.Weapon[i].Label }, position, bp.Weapon[i].DamageRadius, bp.Weapon[i].Damage, bp.Weapon[i].DamageType, bp.Weapon[i].DamageFriendly)
                break
            end
        end
        ...
        z = z + 3
        DamageRing({Unit = self, Proj = nil}, {x,y,z}, 0.1, 3, 1, 'Force', true)
        WaitSeconds(0.5)
