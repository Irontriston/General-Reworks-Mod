...
local statFuncs = {
    function(info)
        ...
        if info.massProduced > 0 or massUsed > 0 then
            return string.format('%+.2f', info.massProduced - massUsed),
        ...
        if UnitData[info.entityId].xp ~= nil then
            ...
            for index = 1, 5 do
                ...
                if UnitData[info.entityId].xp < vet then
                    return string.format('%.2f / %d', UnitData[info.entityId].xp, vet)
                end
            end
    ...
    function(info, bp)
        if info.shieldRatio > 0 then
            return string.format('%.2f%%', info.shieldRatio * 100)
        else
            return false
        end
    end,
...
function UpdateWindow(info)
    if info.blueprintId == 'unknown' then
        ...
        if info.health then
            ...
            controls.health:SetText(string.format("%.2f / %.2f", info.health, info.maxHealth))
        else
            controls.healthBar:Hide()
        end
        ...
        if info.focus then
            ...
            if info.focus.health and info.focus.maxHealth then
                controls.actionText:SetFont(UIUtil.bodyFont, 13)
                controls.actionText:SetText(string.format('%.2f%%', (info.focus.health / info.focus.maxHealth) * 100))
        ...
        elseif info.focusUpgrade then
            ...
            controls.actionText:SetText(string.format('%.2f%%', info.workProgress * 100))
            controls.actionIcon:Show()
            controls.actionText:Show()
    ...
    if options.gui_detailed_unitview ~= 0 then
        if info.blueprintId ~= 'unknown' then
            ...
            if info.userUnit ~= nil then
                ...
                controls.health:SetText(string.format("%.2f / %.2f +%.2f/s", info.health, info.maxHealth, regen))
            end
            ...
            if info.shieldRatio > 0 then
                ...
                if shieldMaxHealth > 0 then
                    local shieldHealth = shieldMaxHealth * info.shieldRatio
                    local shieldText = string.format("%.2f / %.2f", shieldHealth, shieldMaxHealth)
                    if shieldRegenRate > 0 then
                        shieldText = shieldText .. string.format("+%.2f/s", shieldRegenRate)
                    end
                    if shieldMaxHealth > 0 then
                        controls.shieldText:Show()
                        if shieldRegenRate > 0 then
                            controls.shieldText:SetText(string.format("%.2f / %.2f +%.2f/s",
                                shieldMaxHealth * info.shieldRatio, shieldMaxHealth, shieldRegenRate))
                        else
                            controls.shieldText:SetText(string.format("%.2f / %.2f",
                                shieldMaxHealth * info.shieldRatio, shieldMaxHealth))
                        end
                    end
                end
            end
...
function CreateUI()
    ...
    controls.health = UIUtil.CreateText(controls.healthBar, '', 13, UIUtil.bodyFont)
    ...
    if options.gui_detailed_unitview ~= 0 then
        controls.shieldText = UIUtil.CreateText(controls.bg, '', 12, UIUtil.bodyFont)
    end
