...
function ConfigureBeatFunction()
    ...
    local function fmtnum(n)
        return math.clamp(n, 0, 99999999)
    end
    ...
    local function getResourceUpdateFunction(rType, vState, GUI)
        ...
        return function()
            ...
            -- Set the text
            reclaimDelta:SetText(string.format('-%.2f', fmtnum(reclaimRate)) )
            reclaimTotal:SetText(string.format('-%.2f', fmtnum(totalReclaimed)) )
            ...
            curStorage:SetText(string.format('%.2f', storedVal))
            ...
            incomeTxt:SetText(string.format("+%.2f", fmtnum(generatedIncome)))
            expenseTxt:SetText(string.format("-%.2f", fmtnum(expense)))
            ...
            if expense == 0 then
                ...
                effVal = (incomeSec / expense) * 100
            end

            -- Choose to display efficiency or rate
            if States[viewState] == 2 then
                rateTxt:SetText(string.format("%.2f%%", math.min(effVal, 100)))
            else
                rateTxt:SetText(string.format("%+.2f", rateVal))
            end
