...
ADFTractorClaw = ClassWeapon(Weapon) {
    ...
    OnCreate = function(self, spec)
        ...
        -- make us quite a bit slower, but not THAT SLOW
        self.AimControl:SetResetPoseTime(1.0)
    ...
    OnFire = function(self)
        ...
        ---@type Blip | Unit
        local blipOrUnit = self:GetCurrentTarget()
        if not blipOrUnit or--Old code that SC Vanilla uses, pretty useful here, Though disallowing these first is probably better...
            EntityCategoryContains(categories.STRUCTURE, blipOrUnit) or
            EntityCategoryContains(categories.COMMAND, blipOrUnit) or
            EntityCategoryContains(categories.EXPERIMENTAL, blipOrUnit) or
            EntityCategoryContains(categories.NAVAL, blipOrUnit) or
            EntityCategoryContains(categories.SUBCOMMANDER, blipOrUnit) or
            not EntityCategoryContains(categories.ALLUNITS, blipOrUnit) then
			self.Trash:Add(ForkThread(self.OnInvalidTargetThread, self ))
            return
        end
