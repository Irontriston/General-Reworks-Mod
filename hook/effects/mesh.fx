...
float4 NormalMappedInsectPS_02( NORMALMAPPED_VERTEX vertex, uniform bool hiDefShadows) : COLOR0
{
    ...
    light = light * 1.2;
    float phongAmount = saturate( dot( reflect( sunDirection, normal), -vertex.viewDirection));
    float3 phongAdditive = pow( phongAmount, 2) * specular.g * light * 1;
    float3 phongMultiplicative = (phongAmount * environment * specular.r * light * 0.2);
  
    float emissive = glowMultiplier * specular.b * 0.1;

    //Finish it
    float3 color = (albedo.rgb * 0.125) + ( emissive.r + (light * albedo.rgb) ) + phongMultiplicative + (phongAdditive);
    float alpha = mirrored ? 0.5 : min(specular.b, 0.35) + glowMinimum + (phongAdditive * 0.08);
...
{
    float2 tc1 = vertex.texcoord0.xy;
    ...
    float2 tc3 = vertex.texcoord0.xy * 0.02;
    tc3.x -= 0.001 * vertex.material.x;
    float4 lookup3 = tex2D( lookupSampler, tc3);

    float electricity =  lookup.r * lookup2.b * 0.85;
    float4 baseshellcolor = float4( 0.4, 0.7, 1, 1);
...
{
    float2 tc1 = vertex.texcoord0.xy;
    tc1.x += 0.01 * vertex.material.x;
    tc1.y += 0.05 * vertex.material.x;
    float4 lookup = tex2D( secondarySampler, tc1);
    
    float2 tc2 = vertex.texcoord0.xy * 6;
    tc2.y += 0.02 * vertex.material.x;
    tc2.x -= 0.1 * vertex.material.x;
...
>
{
    pass P0
    {
        ...
        VertexShader = compile vs_1_1 PositionNormalOffsetVS(0);
...
technique PBR_Cybran
<
    string abstractTechnique = "PBR_Cybran";
    int fidelity = FIDELITY_HIGH;
