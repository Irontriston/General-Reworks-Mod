...
UEFNukeEffectController01 = Class(NullShell) {
    ...
    OuterRingDamage = function(self)
        ...
        if self.NukeOuterRingTotalTime == 0 then
            DamageArea({ Unit = self.Launcher, Proj = self:GetEntityId() }, myPos, self.NukeOuterRingRadius, self.NukeOuterRingDamage, self.DamageData.DamageType, true, true)
        else
            local ringWidth = ( self.NukeOuterRingRadius / self.NukeOuterRingTicks )
            local tickLength = ( self.NukeOuterRingTotalTime / self.NukeOuterRingTicks )
            -- Since we're not allowed to have an inner radius of 0 in the DamageRing function,
            -- I'm manually executing the first tick of damage with a DamageArea function.
            DamageArea({ Unit = self.Launcher, Proj = self:GetEntityId() }, myPos, ringWidth, self.NukeOuterRingDamage, 'Normal', true, true)
            WaitSeconds(tickLength)
            for i = 2, self.NukeOuterRingTicks do
                --print('Damage Ring: MaxRadius:' .. 2*i)
                DamageRing({ Unit = self.Launcher, Proj = self:GetEntityId() }, myPos, ringWidth * (i - 1), ringWidth * i, self.NukeOuterRingDamage, self.DamageData.DamageType, true, true)
                WaitSeconds(tickLength)
            end
        end
    end,
    ...
    InnerRingDamage = function(self)
        ...
        if self.NukeInnerRingTotalTime == 0 then
            DamageArea({ Unit = self.Launcher, Proj = self:GetEntityId() }, myPos, self.NukeInnerRingRadius, self.NukeInnerRingDamage, self.DamageData.DamageType, true, true)
        else
            local ringWidth = ( self.NukeInnerRingRadius / self.NukeInnerRingTicks )
            local tickLength = ( self.NukeInnerRingTotalTime / self.NukeInnerRingTicks )
            -- Since we're not allowed to have an inner radius of 0 in the DamageRing function,
            -- I'm manually executing the first tick of damage with a DamageArea function.
            DamageArea({ Unit = self.Launcher, Proj = self:GetEntityId() }, myPos, ringWidth, self.NukeInnerRingDamage, 'Normal', true, true)
            WaitSeconds(tickLength)
            for i = 2, self.NukeInnerRingTicks do
                #LOG('Damage Ring: MaxRadius:' .. ringWidth * i)
                DamageRing({ Unit = self.Launcher, Proj = self:GetEntityId() }, myPos, ringWidth * (i - 1), ringWidth * i, self.NukeInnerRingDamage, self.DamageData.DamageType, true, true)
                WaitSeconds(tickLength)
            end
        end
    end,
    ...
    EffectThread = function(self)
        ...
        CreateDecal(position, orientation, 'Crater01_albedo', '', 'Albedo', 75, 75, 1500, 0, army)
        CreateDecal(position, orientation, 'Crater01_normals', '', 'Normals', 75, 75, 1500, 0, army)       
        CreateDecal(position, orientation, 'nuke_scorch_003_albedo', '', 'Albedo', 150, 150, 1500, 0, army)    

    -- Knockdown force rings
        DamageRing({ Unit = self.Launcher, Proj = self:GetEntityId() }, position, 0.1, 150, 1, 'Force', true)
        WaitSeconds(0.1)
        DamageRing({ Unit = self.Launcher, Proj = self:GetEntityId() }, position, 0.1, 200, 1, 'Force', true)
    ...
    CreateInitialFireballSmokeRing = function(self)
        ...
        for i = 0, (sides-1) do
            ...
            self:CreateProjectile('/effects/entities/UEFNukeShockwave01/UEFNukeShockwave01_proj.bp', X * OffsetMod , 1.5, Z * OffsetMod, X, 0, Z):SetVelocity(velocity):SetAcceleration(-0.5)
        end   
    end,  
    ...
    CreateOuterRingWaveSmokeRing = function(self)
        ...
        for i = 0, (sides-1) do
            local X = math.sin(i*angle)
            local Z = math.cos(i*angle)
            local proj =  self:CreateProjectile('/effects/entities/UEFNukeShockwave02/UEFNukeShockwave02_proj.bp', X * OffsetMod , 2.5, Z * OffsetMod, X, 0, Z):SetVelocity(velocity)
    ...
    CreateGroundPlumeConvectionEffects = function(self,army)
		...
		for i = 0, (sides-1) do
			...
			self:CreateProjectile('/effects/entities/UEFNukeEffect05/UEFNukeEffect05_proj.bp', x, RandomFloat(outer_lower_height, outer_upper_height), z, x, 0, z):SetVelocity(x * velocity, 0, z * velocity)
		end 
    end,
}
